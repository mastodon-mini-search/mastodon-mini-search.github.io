...
3. **Search index — `src/functions/createIndex.ts` + `src/functions/tokenize.ts`.** The MiniSearch `options` object in `createIndex.ts` is the single source of truth for the index shape; building and restoring (`MiniSearch.loadJSON`) must use the *same* options, since custom functions like `tokenize` aren't serialized. The index is a **derived, throwaway cache**: `restoreIndex` validates a cached blob (version, document count, decodability) and rebuilds from toots on any mismatch; `indexNewStatuses` grows it incrementally after a fetch. `tokenize` is passed to MiniSearch and runs for **both** indexing and querying, so the two can't drift. **A full (re)build runs in a Web Worker** (`index.worker.ts`, launched by `buildIndex.ts`) so tokenizing the whole corpus doesn't freeze the page. The split is forced by the DOM: `extractDocs` strips HTML on the main thread (a worker has no `document`), the worker only tokenizes+indexes+serializes the pre-stripped docs, and the main thread turns the returned JSON back into a searchable instance via `loadIndexJSON` (a `MiniSearch` carries its `tokenize` fn and can't cross the worker boundary). Only the full build is offloaded; cache restore and incremental `indexNewStatuses` stay on the main thread (cheap).
...
5. **Composables — `src/composables/`.** Reactive orchestration sitting between functions and components. `useSessions` is a **module-level singleton** (shared app-wide session state — `activeStore`, `accounts`, `busy`) so `Main` and `AccountSwitcher` mutate the same state with no event seam. `useSearchIndex` owns the index lifecycle (restore/build/grow/clear); since the build is async (worker), it exposes a `building` flag for the UI and uses a **generation guard** so a build that finishes after the active account changed can't republish a stale index. The worker builder is **injected** (default = `buildPersistedIndex`) so tests build in-process. `useSearch` owns the search box, debounce, committed query, and type filter.
