...
Mastodon 的全文搜索得靠管理員另外架設 [Elasticsearch](https://docs.joinmastodon.org/admin/optional/elasticsearch/)。它很吃資源（要跑 JVM、動輒數 GB 記憶體又得另外維運），不少小型實例索性不開——在這些實例上，連自己的嘟文、喜歡與書籤都無法全文搜索，只剩標籤可搜。本工具把資料抓到瀏覽器本機自建索引，不管實例有沒有 Elasticsearch 都搜得到。
...
- **索引**：手寫的扁平倒排索引（BM25 排序），序列化成 `ArrayBuffer`（排序詞典 + typed-array postings）存進 IndexedDB。冷啟動還原時只是把 buffer 包成 typed-array view（毫秒級），不必重建任何結構；新嘟文增量併入，無需重建整個索引。
- **不卡頓**：建索引與搜索都在 Web Worker 裡跑，主執行緒不被阻塞，UI 不凍結。
...
Vue 3（`<script setup>`）· TypeScript · Vite · Web Worker · [masto.js](https://github.com/neet/masto.js) · [localforage](https://github.com/localForage/localForage)（IndexedDB）· [n-gram](https://github.com/words/n-gram) · Vitest

全文索引是手寫的扁平 `ArrayBuffer` 倒排索引（BM25）；[MiniSearch](https://github.com/lucaong/minisearch) 僅作為測試時比對排序行為的對照基準保留。
